...
#### 新生代
年轻代是所有新对象创建的地方。当填充年轻代时，执行垃圾收集。这种垃圾收集称为 **Minor GC**。年轻一代被分为三个部分——伊甸园（**Eden Memory，包含TLAB**）和两个幸存区（**Survivor Memory**，被称为from/to或s0/s1），默认比例是`8:1:1`
...
4. 永久代的内存占用主要来自类的元数据信息、符号引用、运行时常量池等。而元空间的内存占用只包括类的元数据信息，其他一些数据（如符号引用）会存储在本地内存中。
### 永久代或元空间什么情况下会出现OOM？
- 类的数量过多：在加载大量类的情况下，如果永久代或元空间的大小不足以容纳这些类的元数据信息，就会发生OOM。
- 字符串常量池过大：当应用程序使用大量字符串常量时，字符串常量池可能会占用大量的永久代或元空间，如果空间不足，就会发生OOM。
- 动态代理类过多：使用动态代理创建大量代理类时，这些代理类的元数据信息会存储在永久代或元空间中，如果空间不足，就会发生OOM。
